...
专为树莓派(Raspberry Pi)设计的多媒体播放器，支持(视频、音频、流媒体、图片、摄像头、动画、文本、二维码)
...
23. 支持(文本)自定义(字体大小、字体颜色、背景颜色、透明度、对齐方式、风格样式、多行、等 ...)
14. 支持自定义布局(通过多层功能可以实现多种自定义布局)
15. 支持自定义(视频)是否循环播放(视频在播放到结尾时是否停留在最后一帧)
16. 支持自定义素材尺寸(width，height)，任意拉伸缩放素材尺寸播放
17. 支持自定义素材位置(x，y)播放，任意定义素材播放位置
18. 支持自定义素材横竖屏播放
19. 支持自定义素材开始播放时间(多个播放器间可以实现同步播放)
20. 支持静音播放
